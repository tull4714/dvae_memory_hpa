...
def build_encoder(seq_len, k_orders=(2, 4), q_depth=3):
    ...
    z = layers.Conv1D(64, 1, padding="same")(x)
    return models.Model(inputs, z, name="tcn_physics_encoder")


# -----------------------------
# Decoder (TCN — 훈련 시와 동일 구조)
# -----------------------------
...
name = '/content/drive/MyDrive/NonlinearMemory/curriculum_stage2_epoch40.weights.h5'

# 모델 구조 재생성 후 가중치 로드 (TCN + 물리 기저 — 훈련과 동일)
# k_orders, q_depth는 훈련 파일과 반드시 일치해야 함
encoder = build_encoder(seq_len, k_orders=(2, 4), q_depth=3)
decoder = build_decoder(seq_len)
dvae = DVAE_Curriculum(encoder, decoder, backoff=float(back_off))
dvae.compile(optimizer=tf.keras.optimizers.Adam(1e-3))
dvae.build((None, seq_len, 2))

# ★ forward pass로 모든 레이어 가중치를 완전히 생성한 뒤 로드
# (build만으로는 일부 가중치가 미생성될 수 있어 load_weights가 누락될 위험)
_ = dvae(tf.constant(dvae_1ch[:N]), training=False)

dvae.load_weights(name)
print(f'DVAE_Curriculum 가중치를 {name}에서 성공적으로 로드했습니다.')

# =======================================================================
# 진단 A: 가중치 로드 검증
# 가중치가 0이거나 초기값 그대로면 로드 실패
# =======================================================================
print("\n" + "%"*60)
print("% 진단 A: 가중치 로드 검증 (encoder 레이어별 첫 가중치)")
print("%"*60)
for layer in dvae.encoder.layers:
    if len(layer.weights) > 0:
        w = layer.weights[0].numpy()
        print(f"  {layer.name:28s} shape={str(w.shape):18s} "
              f"mean={w.mean():+.4f} std={w.std():.4f}")
print("  (std가 0이거나 모두 동일하면 로드 실패)")
print("%"*60 + "\n")

# =======================================================================
# 진단 B: 배치 분할(블록 경계) 효과 확인
# causal padding 사용 시 64블록 분할이 결과를 왜곡하는지 검증
# 방식A(64씩) vs 방식B(통째로)가 다르면 블록 경계 문제
# =======================================================================
print("%"*60)
print("% 진단 B: 배치 분할 효과 (causal padding 경계 검증)")
print("%"*60)
_test = dvae_1ch[:100]   # 100개 시퀀스로 테스트

# 방식 A: N(64)개씩 나눠서 처리 (현재 추론 방식)
_outA = []
for i in range(0, len(_test), N):
    _outA.append(dvae(tf.constant(_test[i:i+N]), training=False).numpy())
_outA = np.concatenate(_outA, axis=0)

# 방식 B: 한 번에 전체 처리
_outB = dvae(tf.constant(_test), training=False).numpy()

_diff = np.max(np.abs(_outA - _outB))
print(f"  방식A(64씩) vs 방식B(통째) 최대 차이: {_diff:.8f}")
if _diff < 1e-5:
    print("  ✓ 배치 분할 무관 (블록 경계 문제 없음)")
else:
    print("  ⚠ 배치 분할이 결과를 바꿈 → 처리 방식 문제")
print("%"*60 + "\n")

# =======================================================================
# 진단 C: correction의 시퀀스 내 위치별 분포
# causal padding이면 시퀀스 앞부분(과거 정보 부족)에서 correction이 클 것
# =======================================================================
print("%"*60)
print("% 진단 C: correction의 시퀀스 내 위치별 크기")
print("%"*60)
_pred_sample = dvae(tf.constant(dvae_1ch[:1000]), training=False).numpy()
_corr_sample = _pred_sample - dvae_1ch[:1000]   # (1000, 64, 2)
# 시퀀스 내 위치(0~63)별 correction 크기 평균
_corr_by_pos = np.sqrt(np.mean(_corr_sample**2, axis=(0, 2)))  # (64,)
print(f"  위치 0~7   평균 correction: {np.mean(_corr_by_pos[:8]):.6f}")
print(f"  위치 8~15  평균 correction: {np.mean(_corr_by_pos[8:16]):.6f}")
print(f"  위치 16~31 평균 correction: {np.mean(_corr_by_pos[16:32]):.6f}")
print(f"  위치 32~63 평균 correction: {np.mean(_corr_by_pos[32:]):.6f}")
print(f"  → 앞부분(0~7)이 뒷부분(32~63)보다 크게 높으면 causal 경계 문제")
print("%"*60 + "\n")

# =======================================================================
# 진단 1: 추론 입력 단계 RMS 확인
# 훈련 시 X_in_train_ch RMS(≈1)와 비교 — 다르면 정규화 스케일 불일치
# =======================================================================
print("\n" + "="*55)
print("추론 파이프라인 진단")
print("="*55)
print(f"추론 rms_mag (정규화 기준):  {rms_mag:.6f}")
print(f"  → 훈련 로그의 rms_train 값과 비교하세요")
print(f"IQ_out_r RMS:               {np.sqrt(np.mean(np.abs(IQ_out_r)**2)):.6f}")
print(f"dvae_1ch 입력 RMS:          {np.sqrt(np.mean(dvae_1ch**2)):.6f}  (정상: ~1.0)")

# DLA 추론: 원본 신호 → predistorted 신호
# DVAE_Curriculum.call()은 predistorted 단일 텐서를 반환
sig_in_Linear_list = []
for i in range(0, len(dvae_1ch), N):
    batch = tf.constant(dvae_1ch[i:i+N])
    predistorted = dvae(batch, training=False)   # 단일 출력
    sig_in_Linear_list.append(predistorted.numpy())
sig_in_Linear = np.concatenate(sig_in_Linear_list, axis=0)

# =======================================================================
# 진단 2: 모델 출력 및 correction 크기 확인
# correction RMS가 훈련 로그(≈0.047)와 비슷해야 정상
# 매우 크면(>1) 모델 추론 자체가 망가진 것
# =======================================================================
correction_2ch = sig_in_Linear - dvae_1ch
print(f"predistorted 출력 RMS:      {np.sqrt(np.mean(sig_in_Linear**2)):.6f}  (정상: ~1.0)")
print(f"correction RMS:             {np.sqrt(np.mean(correction_2ch**2)):.6f}  (정상: ~0.047)")
