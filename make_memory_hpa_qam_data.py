...
def gen_mapping(M, D):
    ...
    if M == 2:
        ...
        # 16QAM: I, Q ∈ {-3, -1, +1, +3}, 평균 심볼 에너지 = 10
        s = (-2 * np.fix(rand_I * 4) + 3) + 1j * (-2 * np.fix(rand_Q * 4) + 3)
        # sqrt(10)으로 정규화 → 단위 전력(평균전력 1), 표준 정규화
        s = s / np.sqrt(10.0)
...
# mode='w'로 변경 — 이전 데이터(back_off=0 등)와 섞이지 않도록 덮어쓰기
with open("/content/drive/MyDrive/input_target_I.csv", mode='w', newline='') as go2_I:
...
with open("/content/drive/MyDrive/input_target_Q.csv", mode='w', newline='') as go2_Q:
...
# Add AWGN (numpy 사용 — tf 미import 버그 수정)
# snr_db < 0 이면 노이즈 없음.
# Supervised 학습에서 correction_target = 원본 - HPA(원본)이므로
# HPA 출력에 노이즈가 섞이면 학습 목표가 흐려진다.
# 노이즈는 추론 시 채널에서 추가되므로 훈련 데이터는 깨끗하게 둔다.
snr_db = -1.0
if snr_db >= 0.0:
    signal_power = np.mean(Y_I_distorted**2 + Y_Q_distorted**2)
    snr_linear   = 10**(snr_db / 10)
    noise_power  = signal_power / snr_linear
    noise_std    = np.sqrt(noise_power / 2)

    Y_I_distorted = Y_I_distorted + np.random.normal(
        loc=0.0, scale=noise_std, size=Y_I_distorted.shape)
    Y_Q_distorted = Y_Q_distorted + np.random.normal(
        loc=0.0, scale=noise_std, size=Y_Q_distorted.shape)
    print(f"HPA(x) + n, SNR = {snr_db}\n")
else:
    print("HPA(x) — 노이즈 없음 (깨끗한 학습 데이터)\n")
with open("/content/drive/MyDrive/input_iq_I.csv", mode='w', newline='') as go1_I:
...
with open("/content/drive/MyDrive/input_iq_Q.csv", mode='w', newline='') as go1_Q:
